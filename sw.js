...
const CACHE_NAME = "eatwithme-shell-v75";
const APP_SHELL = [
  "./",
  "./index.html",
  "./styles.css",
  ...
  if (NETWORK_FIRST_PATHS.has(requestUrl.pathname)) {
    event.respondWith(
      fetch(event.request).then((response) => {
        if (response.ok && (requestUrl.origin === self.location.origin || requestUrl.hostname === "unpkg.com" || requestUrl.hostname.endsWith("openstreetmap.org"))) {
